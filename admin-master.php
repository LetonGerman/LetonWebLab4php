...
<header class="container-fluid">
    <div class="row navbar-dark bg-light justify-content-center">
        ...
        <nav class="nav nav-pills flex-row flex-md-row flex-lg-row flex-xl-row justify-content-right">
            <a class="nav-link flex-sm-fill text-sm-center" href="phplab.project17.ru">Main page</a>
        </nav>
    </div>
</header>
...
if(isset($_POST["add"])) {
    ...
    mysqli_select_db($conn , 'news');
    $now = time();
    $headline = mysqli_real_escape_string($conn, $_POST['article-id']);
    $sql = "INSERT IGNORE INTO articles (headline, text, datePublished) VALUES ('" . $headline . "', '" . $_POST["body"] . "', '" . $now . "')";
...
// prevent SQL injection
    ...
    $res = mysqli_query($conn,'DELETE FROM articles WHERE id='.$articleId);
